...
let UsersController = class UsersController {
    ...
    async updateSession({ name }, session, { permission }) {
        const user = await this.usersService.findByUsername(name);
        if (user && bcrypt.compareSync(user.password, permission)) {
            session.user = user;
            return user;
        }
        throw new common_1.HttpException(name, common_1.HttpStatus.NOT_FOUND);
    }
    remove(usernam) {
...
__decorate([
    (0, common_1.Patch)('/reconnect/:name'),
    ...
    __metadata("design:returntype", Promise)
], UsersController.prototype, "updateSession", null);
__decorate([
